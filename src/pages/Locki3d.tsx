...
import { DataNftCard, Loader, Locki3DModal, TrailBlazerModal } from "components";
import { LOCKI3D_NONCES } from "config";
import { useGetAccount, useGetPendingTransactions } from "hooks";
import { toastError } from "libs/utils";
import "react-vertical-timeline-component/style.min.css";
import { HeaderComponent } from "../components/Layout/HeaderComponent";

export const Locki3d = () => {
  ...
  async function fetchAppNfts() {
    ...
    const _nfts: DataNft[] = await DataNft.createManyFromApi(LOCKI3D_NONCES.map((v) => ({ nonce: v })));
  ...
  return (
    <HeaderComponent
      pageTitle={"Locki 3D NFT"}
      hasImage={true}
      imgSrc={headerHero}
      altImageAttribute={"locki3d"}
      ...
      {itDataNfts.length > 0 ? (
        ...
        <Locki3DModal isModalOpened={isModalOpened} closeModal={closeModal} owned={owned} isFetchingDataMarshal={isFetchingDataMarshal} data={data} />
    </HeaderComponent>
  );
};
